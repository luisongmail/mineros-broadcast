...
function PitcherCard({ pitcher, assetBaseUrl }: { pitcher: ScoreboardPitcherLine; assetBaseUrl?: string }) {
  ...
  return (
    <div className="flex-1 rounded-[6px] border border-white/8 bg-black/20 px-4 py-3">
      <div className="flex items-center gap-3">
        <TeamBadge abbr={pitcher.teamAbbr} assetBaseUrl={assetBaseUrl} logoAssetId={pitcher.teamLogoAssetId} size="md" />
        <div className="min-w-0 flex-1">
          <div className="truncate font-bebas text-[22px] uppercase leading-none tracking-[0.03em] text-white">
            {pitcher.playerNumber ? `#${pitcher.playerNumber} ` : ''}
            {pitcher.playerName}
          </div>
          <div className="mt-1.5 flex flex-wrap gap-x-3 gap-y-1 text-[13px] font-semibold text-[#9CA3AF]">
            {metrics.map((metric) => (
              <span key={`${pitcher.playerId}-${metric}`}>{metric}</span>
            ))}
            <span className="font-bebas text-[20px] leading-none text-[#D4AF37]">{pitcher.pitchCount ?? '--'} PIT</span>
          </div>
        </div>
      </div>
    </div>
...
export function Zone4Pitchers({ data, assetBaseUrl }: { data: ScoreboardOverlayData; assetBaseUrl?: string }) {
  return (
    <section className="flex h-full flex-col overflow-hidden rounded-[6px] border border-white/8 bg-[#111111] px-4 py-3 shadow-[0px_2px_8px_rgba(0,0,0,.25)]">
      <div className="text-[11px] font-bold uppercase tracking-[0.24em] text-[#D4AF37]">Lanzadores</div>
